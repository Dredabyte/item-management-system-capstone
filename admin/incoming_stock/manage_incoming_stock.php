<?php 
if(isset($_GET['id'])){
    $qry = $conn->query("SELECT p.* FROM incoming_list p where p.id = '{$_GET['id']}'");
    if($qry->num_rows >0){
        ...
        if($from_order == 1){
            ...
            $qry = $conn->query("SELECT b.*,s.name as supplier,p.po_code FROM backorder_list b inner join supplier_list s on b.supplier_id = s.id inner join purchase_order_list p on b.po_id = p.id  where b.id = '{$_GET['bo_id']}'");
            if($qry->num_rows >0){
                foreach($qry->fetch_array() as $k => $v){
                    if($k == 'id')
                    $k = 'bo_id';
...
<div class="card card-outline card-primary">
    <div class="card-header">
        <h4 class="card-title"><?php echo !isset($id) ? "Incoming Order from ".$po_code : 'Update Incoming Order' ?></h4>
    </div>
    <div class="card-body">
            ...
            <div class="container-fluid">
                ...
                <table class="table table-striped table-bordered" id="list">
                    ...
                    <tbody>
                        ...
                        $qry = $conn->query("SELECT b.*,i.name,i.description FROM `backorder_items` b inner join item_list i on b.item_id = i.id where b.bo_id = '$bo_id'");
    ...
    <div class="card-footer py-1 text-center">
        <button class="btn btn btn-primary" type="submit" form="receive-form">Save</button>
...
<script>
    $(function(){
        ...
        $('#receive-form').submit(function(e){
			...
			start_loader();
			$.ajax({
				url:_base_url_+"classes/Master.php?f=save_incoming",
				...
				success:function(resp){
					if(resp.status == 'success'){
						location.replace(_base_url_+"admin/?page=incoming_stock/view_incoming_stock&id="+resp.id);

...
Class Users extends DBConnection {
	...
	public function save_users(){
		...
		foreach($_POST as $k => $v){
			if(in_array($k,array('firstname','middlename','lastname','username','role','email','type'))){
				if(!empty($data)) $data .=" , ";
				$data .= " {$k} = '{$v}' ";
			}
		}
		...
		if($resp['status'] == 1){
			$data="";
			foreach($_POST as $k => $v){
				if(!in_array($k,array('id','firstname','middlename','lastname','username','role','email','password','type','oldpassword'))){
					if(!empty($data)) $data .=", ";
